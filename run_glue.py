...
from models import BertForSequenceClassification,BertWhou
...
MODEL_CLASSES = {
    'bert': (BertConfig, BertWhou, BertTokenizer),
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, type='evaluate')
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for predict_task, predict_output_dir in zip(predict_task_names, predict_outputs_dirs):
        predict_dataset = load_and_cache_examples(args, predict_task, tokenizer, type='predict')
        ...
        pickle.dump(preds,open(output_predict_file,'wb'))

    return None

def load_and_cache_examples(args, task, tokenizer, type='train'):
    if args.local_rank not in [-1, 0] and type=='train':
    ...
    if args.local_rank == 0 and type=='train':
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
...
def main():
    ...
    parser.add_argument("--per_gpu_predict_batch_size", default=128, type=int,
